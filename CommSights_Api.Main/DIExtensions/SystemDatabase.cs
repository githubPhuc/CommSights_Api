...
namespace CommSights_Api.Main.DIExtensions
{
    public static class SystemDatabase
    {
        public static IServiceCollection AddSystemDatabase(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            services.AddDbContext<CommSightsContext>(options => options.UseSqlServer(configuration.GetConnectionString("CommSightsContext") ?? throw new InvalidOperationException("Connection string 'CommSightsContext' not found.")),ServiceLifetime.Scoped);
            return services;
        }
    }
}
